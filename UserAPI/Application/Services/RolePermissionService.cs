...
public class RolePermissionService : IRolePermissionService
{
    ...
    public async Task<List<GetRolePermissionResponse>> GetAllRolePermissions(){
        var rolePermissions = await _context.RolePermissions
            ...
            .OrderBy(rp => rp.Role.Name).ToListAsync();
        //if(rolePermissions == null) return new List<GetRolePermissionResponse>();
        return rolePermissions.Select(rp => GetRolePermissionResponse.ToResponse(rp.RoleId, rp.Role.Name, rp.PermissionId, rp.Permission.Name)).ToList();
    }
    ...
    public async Task<List<GetRolePermissionResponse>> GetRolePermissionById(int roleId){
        ...
        //if (rolePermissions == null) return new List<GetRolePermissionResponse>();
        return rolePermissions.Where(rp => rp.Role.RoleId == roleId).Select(rp => GetRolePermissionResponse.ToResponse(rp.RoleId, rp.Role.Name, rp.PermissionId, rp.Permission.Name)).ToList();
    }
    ...
    public async Task<List<GetRolePermissionResponse>> GetRolePermissionByName(string roleName){
        ...
        if(rolePermissions == null) return new List<GetRolePermissionResponse>();
        return rolePermissions.Where(rp => rp.Role.Name == roleName).Select(rp => GetRolePermissionResponse.ToResponse(rp.RoleId, rp.Role.Name, rp.PermissionId, rp.Permission.Name)).ToList();
    }

    public async Task<CreateRolePermissionResponse> CreateRolePermission(int roleId, int permissionId){
        
        var role = await _context.Roles
            .FirstOrDefaultAsync(r => r.RoleId == roleId);
        
        if(role == null)
            return CreateRolePermissionResponse.ToRoleNotFoundResponse(roleId);

        var permission = await _context.Permissions
            .FirstOrDefaultAsync(p => p.PermissionId == permissionId);
        
        if(permission == null)
            return CreateRolePermissionResponse.ToPermissionNotFoundResponse(permissionId);

        if(await _context.RolePermissions.AnyAsync(rp => rp.RoleId == role.RoleId && rp.PermissionId == permission.PermissionId))
        {
            return CreateRolePermissionResponse.ToSuccessResponse(
                await _roleService.GetAllRolePermissions(role.RoleId)
            );
        }
    ...
    //     _context.RolePermissions.Add(newRolePermission);
    //     await _context.SaveChangesAsync();
    //     return GetRolePermissionResponse.ToResponse(role.RoleId, role.Name, permission.PermissionId, permission.Name);
    // }

    public async Task<UpdateRolePermissionResponse> UpdateRolePermission(CreateRolePermissionRequest rolePermission){
        var role = await _context.Roles
        ...
        if (rolePermissionExists)
            return UpdateRolePermissionResponse.ToUpdateResponse();
